    ...
    try {
        ...
        switch (method) {
            case httpConstants.HTTP2_METHOD_GET: {
                const result = await coffeeService.getBrewMethodById(id);
            ...
            case httpConstants.HTTP2_METHOD_PUT: {
                ...
                const result = await coffeeService.updateBrewMethod(id, body);
                return res.status(httpConstants.HTTP_STATUS_OK).json({data: result});
            }
            case httpConstants.HTTP2_METHOD_DELETE: {
                const result = await coffeeService.deleteBrewMethod(id);
                return res.status(httpConstants.HTTP_STATUS_NO_CONTENT).end();
            }
            default: {
                res.status(httpConstants.HTTP_STATUS_BAD_GATEWAY).json({data: "Method not supported"})
            }
        }
    } catch (error) {
