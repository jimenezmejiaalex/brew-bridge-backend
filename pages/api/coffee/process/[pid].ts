    ...
    try {
        ...
        switch (method) {
            case httpConstants.HTTP2_METHOD_GET: {
                const result = await coffeeService.getCoffeeProcessingMethod(id);
            ...
            case httpConstants.HTTP2_METHOD_PUT: {
                ...
                const result = await coffeeService.updateCoffeeProcessingMethod(id, body);
                return res.status(httpConstants.HTTP_STATUS_OK).json({data: result});
            }
            case httpConstants.HTTP2_METHOD_DELETE: {
                const result = await coffeeService.deleteCoffeeProcessingMethod(id);
