    ...
    try {
        ...
        switch (method) {
            case httpConstants.HTTP2_METHOD_GET: {
                const result = await coffeeService.getCoffeeRegionById(id);
            ...
            case httpConstants.HTTP2_METHOD_PUT: {
                ...
                const result = await coffeeService.updateCoffeeRegion(id, body);
                return res.status(httpConstants.HTTP_STATUS_OK).json({data: result});
            }
            case httpConstants.HTTP2_METHOD_DELETE: {
                const result = await coffeeService.deleteCoffeeRegion(id);
