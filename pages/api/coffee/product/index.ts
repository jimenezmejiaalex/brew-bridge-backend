    ...
    try {
        switch (method) {
            case httpConstants.HTTP2_METHOD_POST: {
                const body = req.body;
                const coffeeProduct = body as CoffeeProduct;
                const result = await coffeeService.createCoffeeProduct(coffeeProduct);
                return res.status(httpConstants.HTTP_STATUS_OK).json({data: result});
            }
            case httpConstants.HTTP2_METHOD_GET: {
                const result = await coffeeService.listCoffeeProducts();
                return res.status(httpConstants.HTTP_STATUS_OK).json({data: result});
            }
        }
