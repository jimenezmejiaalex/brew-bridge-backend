    ...
    try {
        ...
        switch (method) {
            case httpConstants.HTTP2_METHOD_GET: {
                const result = await roasterService.getRoasterById(id);
            ...
            case httpConstants.HTTP2_METHOD_PUT: {
                ...
                const result = await roasterService.updateRoaster(id, body as Roaster);
                return res.status(httpConstants.HTTP_STATUS_OK).json({data: result});
            }
            case httpConstants.HTTP2_METHOD_DELETE: {
                const result = await roasterService.deleteRoaster(id);
